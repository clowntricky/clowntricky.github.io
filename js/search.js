...
function search() {
	...
	if (host.indexOf("blog.mumei.cc") > -1) {
		var domain = host.split('/'); //以“/”进行分割
		if (domain[2]) {
			mulu = domain[2] + "/cloud/home";
		}
		var target = "target='iframe'";
		if (path.indexOf("/Blog") > -1 && path.indexOf("/Private") == -1) {
			var action = 'blog';
		} else if (path.indexOf("/Blog/Private") > -1) {
			var action = 'blog_p';
		} else {
			var action = 'home';
		}
	} else {
		var domain = where.split('/');
		if (domain[2]) {
			mulu = domain[2];
		} else {
			alert('未获取到搜索源');
			...
			if (res.length == 0) {
				...
				res.forEach(function(i, n) {
					html1 +=
						...
						mulu + i.link + "' onclick='load()'" + target +
